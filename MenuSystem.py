...
class Menu(Rect,object):
    ...
    def __init__(self,label,itemslist,exc=()):
        ...
        print 'Menu: ' + repr(self.itemsrect)
...
class MenuBar(MenuSystem,object):
    ...
    def set(self,menuboxlist=None):
		...
		if menuboxlist != None:
			...
			'''
			w = 60
			self.rects.append(Rect(x,0,w,self.lineheigth))
		else:
			print 'in else part'
			self.menuboxlist = None
			self.rects = []
		self.index = -1
		self.draw()
		self.choice = None
    ...
    def draw(self):
		...
		print repr(self.menuboxlist)
		if self.menuboxlist != None:
			#for item in self.menuboxlist:
			x = DISPLAY.blit(FONT.render(self.menuboxlist.label,1,FGCOLOR),(x,self.rect.y)).right+self.lineheigth
		return self.rect
